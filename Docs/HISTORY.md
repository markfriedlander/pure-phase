...
## May 7-8, 2026 — Sessions 7-8: 1.0 ship + 2.0 build (Claude Code)

**Session 7 (May 7 day) — App Store submission for 1.0**

End-to-end App Store Connect submission flow, driven via Chrome MCP. Apple Developer license re-accepted. Bundle ID `com.MarkFriedlander.PurePhase` registered. New app listing created (Pure Phase / iOS / SKU `purephase` / Full Access). Categories: Health & Fitness primary, Lifestyle secondary. Subtitle "Breathe. Focus. Calm. Sleep." Content Rights → No. App Privacy → Data Not Collected (published). Pricing → Free, 175 countries. Version 1.0 metadata: description (Strategic Claude's draft), keywords, promotional text, Support and Privacy URLs. Age Rating questionnaire → 18+ override (stroboscopic effects). Regulated Medical Device → No. Build uploaded via Xcode → Archive → Distribute App. App Privacy questionnaire updated. Add for Review failed silently three times due to a hidden "items required" check — fixed by completing Contact Information and toggling Sign-In Required off, then a "type a character + delete" trick to commit JS-set values to React state. Apple's first response was a Guideline 2.1 "Information Needed" rather than a hard rejection — replied with a screen recording from Mark's iPhone 16 Plus and the seven-point context Apple asked for. Apple turned around overnight: 1.0 approved and live on the App Store May 8 morning.

Lessons captured in `Docs/AppStoreSubmission.md` (10 gotchas to remember next time, including iPhone screenshot dimensions, the silent submit-validation check, the React onChange trick, and the trusted-pointer requirement on the Add for Review button).

**Session 8 (May 7 evening through May 8 early morning) — 2.0 build**

Pre-2.0 hygiene pass (commit `c653dc9`): zero build warnings via Swift 6 actor-isolation fixes (`nonisolated` on `BrainwaveState.isBreathwork`/`isCustom` and `Color(hex:)`). Vertical centering modifier `CenteredScrollContainer` for big screens. Photosensitive copy updated to mention bystanders. Catalyst flag stripped. App Review Notes pre-filled in App Store Connect.

PRISM rename (was Psychedelic): one-syllable name, less loaded App Store optics, behavior bit-identical. Touched BrainwaveState, SessionView, AdvancedView, IntentTileView, AutomationServer, SessionConfigView, SessionEngine, CLAUDE.md.

DRIFT audio engine — new `Engine/DriftAudioEngine.swift` (~290 lines). Architecture per CC's pre-build response: three separate `AVAudioSourceNode` instances (one for main carrier, two for Layer 3 shimmer carriers when enabled) feeding `AVAudioMixerNode`. Shared `DriftSynthesisState` (`@unchecked Sendable`, word-aligned Double phase fields, atomic at the hardware level on Apple silicon, no locking). Per-source persistent state captured in render closure: phase accumulators (avoiding the precision drift of `sin(2π × f × t)` for large t) plus a 1024-entry ring buffer for Layer 2 phase-drift delay with linear interpolation on the R channel. First implementation overloaded the iOS simulator's audio thread (libm `sin` × 4 per sample). Optimized via a 4096-entry sine LUT (one-time file-scoped init, ~16 KB, linearly interpolated; cosine derives from same LUT via the identity `cos(p) = sin(p + 0.25)`) plus the phase-accumulator pattern. Verified in sim past 55 seconds without overload. Mark hardware-tested on iPhone 16 Plus + AirPods: "incredibly beautiful and sound amazing." Audio also accidentally verified through Mac speakers (sim audio routes through Mac).

BLOOM visual layer — new `Views/BloomBackgroundView.swift`. SwiftUI `TimelineView(.animation)` + `Canvas` (iOS 17 compatible; `MeshGradient` is iOS 18+). Reads `DriftSynthesisState` LFO phases each frame and draws a full-screen radial gradient with three modulated elements: bloom radius/opacity from the breathing-carrier LFO, color temperature from the phase-drift LFO (interpolating between an amber-leaning palette and a red-leaning palette via sRGB component lerp), and an opt-in shimmer ring overlay from the harmonic-shimmer phase. Movement parameters tuned per Mark's first-listen feedback: radius ±0.15 → ±0.25, opacity ±0.075 → ±0.15, and a NEW ±5% center drift tied to the phase-drift LFO (synesthetic — visual field drifts through space at the same rate audio drifts through the head).

Shared DRIFT/BLOOM config screen — new branch in `Views/SessionConfigView.swift` for `state.usesDriftAudioEngine`. Three layer toggles with descriptions, carrier slider (80-440 Hz, default 220), "Best with headphones" subtitle, ambient picker visibly disabled with the "DRIFT replaces ambient texture" copy. Reuses existing breath/duration sections. Settings persisted via four new `StorageKey` entries; `StorageDefault.restoreAll()` extended.

tvOS target — new `NeuroLightTV` target sharing same bundle ID as iOS so App Store treats both as one listing. `PBXFileSystemSynchronizedRootGroup` for `NeuroLightTV/` plus a second sync group reference on the iOS source folder so shared code (engine, models, audio, BloomBackgroundView, BreathGuideView) compiles into both targets. iOS-only views (HomeView, AdvancedView, SessionConfigView, SessionView, OnboardingView, TileGesture, ContentView, NeuroLightApp) wrapped with `#if os(iOS)` so they're absent from the tvOS target. New TV-specific files: `TVRootView` (router with onboarding gate + session presentation), `TVHomeView` (six tiles arranged horizontally per spec), `TVTileButton` (focus-aware via `Environment(\.isFocused)` with lift/glow), `TVOnboardingView` (focus-default ENTER button, expanded bystander warning copy), `TVSessionView` (full-bleed, reuses SessionEngine + BloomBackgroundView + BreathGuideView, exits on Menu/PlayPause). AutomationServer wired into `NeuroLightTVApp` for screenshot-driving and verification. Real Apple TV pairing failed (bedroom HD 4th gen capped at tvOS 16; living room 4K 2nd gen wouldn't expose Developer toggle). Decision: ship anyway; Apple review will catch any tvOS regressions; living room Apple TV will install via App Store auto-install once 2.0 is approved.

TV screenshots captured at 4K (3840×2160) in `Docs/AppStoreScreenshots/tv-4k/` — home + DRIFT + 2 BLOOM stills, ready for App Store Connect upload.

AirPlay verified as a free zero-code feature: iOS Control Center → Screen Mirroring sends both video and audio of the BLOOM session to any AirPlay receiver, including Mark's bedroom Apple TV / projector. Solves the "I want BLOOM on the wall" use case without a code change. In-app `AVRoutePickerView` polish queued for next session.

**3.0 ideas captured** in `Docs/NEXT.md`: adventure further into layer math (sonic + visual cross-modulation), interference-pattern visual mode (Mark's lifelong fascination with moiré, reskinned to fit the warm aesthetic), possible biofeedback/ADAPTIVE revisit if a real validation path appears.

**Build state at session end:** zero warnings on iOS Simulator + tvOS 26.4 Simulator. 17 commits ahead of origin/main since `64e58aa` (1.0 submission commit).
