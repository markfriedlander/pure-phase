...
### What's left for 2.0 — May 8 afternoon update

Most of yesterday's "deferred" list landed today. Current state:

| Item | Status |
|---|---|
| TV graphics (layered icon + top shelf hero) | ✅ generated by `scripts/render_tvos_icons.swift`, committed to `NeuroLightTV/Assets.xcassets/`, verified on tvOS sim home screen |
| TV screenshots at 4K | ✅ four stills in `Docs/AppStoreScreenshots/tv-4k/` |
| AirPlay (free path: Control Center → Screen Mirroring) | ✅ verified by Mark on his iPhone → Apple TV; ships as-is |
| AirPlay route picker in session view | ❌ dropped — free path is sufficient, in-app picker noted for future polish only if users ask |
| AM rate decision for DRIFT/BLOOM | ✅ Strategic Claude consulted; settled on **2 Hz** (heartbeat tempo). Mark hardware-verified; reaction "it sounds like a heartbeat" matched the design intent. No configurability knob. |
| 2.0 submission copy (Description, Promo, What's New, Keywords) | ✅ Strategic-Claude-approved drafts in `Docs/PurePhase2.0Submission.md` |
| Optional: extend `scripts/capture_screenshots.sh` for tvOS | ⏸ skipped for 2.0 — manual sim drive worked once, can do post-ship if it becomes recurring work |

**The ONE remaining 2.0 item: App Store Connect submission paperwork.**

Pre-flight done. Mark needs to do two Xcode Archives (iOS scheme + tvOS scheme, both → Distribute App → Upload). CC drives App Store Connect via Chrome MCP to set up the version 2.0 entry, **add tvOS as a platform on the EXISTING listing (critical — same bundle ID, "Add Platform → tvOS", do this BEFORE the tvOS binary uploads so it lands in the right slot)**, paste the approved copy, upload screenshots, attach binaries, submit. Full step-by-step flow in `Docs/PurePhase2.0Submission.md`. 1.0 gotchas still apply — see `Docs/AppStoreSubmission.md` (especially the silent "items required" check on Submit, and the React-state "type-and-delete" trick).
