...
- **Heart-rate-driven AM rate for DRIFT/BLOOM** — Mark's reaction to the 2Hz default during 2.0 testing was "this is when I wish we'd connected it to my watch and used my heartbeat as input." The current AM rate is fixed at 2Hz, which gives a heartbeat-like pulse — the natural extension is to make it actually be the user's heartbeat. AM rate = current HR / 60 (so 60bpm → 1Hz, 90bpm → 1.5Hz). Closed-loop synesthesia: the audio breathes with the listener. Pairs naturally with the deferred ADAPTIVE direction since both depend on HealthKit + HKWorkoutSession HR streaming. Probably the single most compelling 3.0 feature.
- **AM rate as user-configurable slider** — Mark also liked 8Hz when he tested it. If users ask post-2.0 launch we add a 1–8Hz slider in the DRIFT/BLOOM config. Below the heart-rate idea in priority — same itch, less interesting solution.
